...
export default function Checkout({ token, cartItems, totalAmount, onClose, onOrderComplete }) {
    ...
    setTimeout(() => {
      ...
      if (currentStep === 2) {
        if (data.paymentMethod === 'cod') {
          nextStep = 4; // Skip payment steps and go to OrderSummary
        } else if (data.paymentMethod === 'razorpay') {
          nextStep = 3; // Go to Razorpay payment
        } else {
          nextStep = 3; // Go to PaymentQR for other methods
        }
      }
  ...
  function renderCurrentStep() {
    ...
    // Handle step 3 dynamically based on payment method
    if (currentStep === 3) {
      if (orderData.paymentMethod === 'razorpay') {
        return (
          <RazorpayPayment
            orderData={{...orderData, totalAmount, userEmail: user?.email}}
            onPaymentSuccess={(paymentDetails) => {
              console.log('Payment successful:', paymentDetails);
              const updatedOrderData = { ...orderData, paymentDetails, paymentStatus: 'completed' };
              setOrderData(updatedOrderData);
              setCurrentStep(4); // Go to OrderSummary
            }}
        ...
        // Use PaymentQR for other methods
        return (
          <PaymentQR
            token={token}
            user={user}
